...
//--- Proximity alert
input double InpProximityPips = 50.0;   // Alert when price is within X pips of a line (0 = disable)
...
//+------------------------------------------------------------------+
//| MQL5 does not expose profile switching from EA code              |
//| Keep a helper so existing logic compiles and fails gracefully    |
//+------------------------------------------------------------------+
bool TryLoadProfile(const string profileName) {
   PrintFormat("[WARN] Profile auto-load is not supported in MQL5 EAs. Requested profile: %s", profileName);
   return false;
}
...
double GetLinePrice(long chartId, string objName, datetime t) {
   ...
   if(objType == OBJ_TREND || objType == OBJ_TRENDBYANGLE || objType == OBJ_REGRESSION) {
      ...
      if(t2 == t1)
         return p1;   // vertical – return anchor price
...
int ClassifyObject(long chartId, string objName, double currentPrice) {
   ...
   // --- Colour filter ---
   if(InpPressureColor != 0 && objColor == InpPressureColor)
      return 1;
   if(InpSupportColor != 0 && objColor == InpSupportColor)
      return -1;
   ...
   if(linePrice > currentPrice)
      return 1;   // above price → pressure
   if(linePrice < currentPrice)
      return -1;   // below price → support
...
void CheckBreakouts(long chartId) {
   ...
   for(int i = 0; i < totalObjects; i++) {
      ...
      // -- Proximity zone: linePrice ± InpProximityPips * Point --
      double proximityDist = (InpProximityPips > 0.0) ? InpProximityPips * _Point : 0.0;
      ...
      if(role == 1)   // PRESSURE line
      {
         ...
         nearLine = (!brokeOut && InpProximityPips > 0.0 && MathAbs(ask - linePrice) <= proximityDist);
      } else if(role == -1)   // SUPPORT line
      {
         brokeOut = (bid < linePrice - buffer);
         nearLine = (!brokeOut && InpProximityPips > 0.0 && MathAbs(bid - linePrice) <= proximityDist);
      }
      ...
      if(nearLine) {
         ...
         if(StringFind(g_alertedProximity, key) < 0) {
            ...
            double refPrice     = (role == 1) ? ask : bid;
            double pipsAway     = MathAbs(refPrice - linePrice) / _Point;
            PrintFormat("[NEAR] %s %s - Price is %.1f pips away from %s line \"%s\"  "
                        "| Mid=%.5f  Line=%.5f  (within %.1f-pip zone)",
                        _Symbol, EnumToString((ENUM_TIMEFRAMES)_Period),
                        pipsAway, roleStr, objName,
                        (ask + bid) / 2.0, linePrice, InpProximityPips);
         }
      }
   }
}
...
int OnInit() {
   ...
   if(InpAutoLoadProfile && StringLen(InpProfileName) > 0) {
      if(TryLoadProfile(InpProfileName))
