...
input int    InpInitialStep  = 1;      // Initial anti-martingale step
...
bool OpenTrade(const int signal) {
   ...
   double requestedLot = baseLot * (double)g_step;
   double lot          = NormalizeVolume(g_tradeSymbol, requestedLot);
   if(lot <= 0.0) {
      PrintFormat("[ERROR] Invalid lot. signal=%d requested=%.6f step=%d", signal, requestedLot, g_step);
      return false;
   }
...
int OnInit() {
   ...
   g_step = MathMin(InpMaxStep, MathMax(InpMinStep, InpInitialStep));

   g_trade.SetExpertMagicNumber(InpMagicNumber);
   g_trade.SetDeviationInPoints(InpDeviationPoint);

   PrintFormat("[INIT] barGambling_lowHigh_antiMartingale on %s %s | BearPct=%.1f BullPct=%.1f Near=%.1f pips | BaseShort=%.2f BaseLong=%.2f | Step=%d [%d..%d] | SL=%.1f TP=%.1f pips",
               g_tradeSymbol,
...
void OnTradeTransaction(const MqlTradeTransaction &trans,
   ...
   if(net > 0.0) {
      g_step = MathMin(InpMaxStep, g_step + 1);
      PrintFormat("[ANTI] Win %.2f -> step = %d", net, g_step);
   } else if(net < 0.0) {
      g_step = MathMax(InpMinStep, g_step - 1);
      PrintFormat("[ANTI] Loss %.2f -> step = %d", net, g_step);
   }
}
