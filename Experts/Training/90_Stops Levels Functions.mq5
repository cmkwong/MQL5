...
int OnInit()
  {
   ...
   Print("What is _Point? ", _Point);     // 2025.07.18 22:06:51.776	90_Stops Levels Functions (NZDUSD,M15)	What is _Point? 0.00001
   Print("What is _Digits? ", _Digits);   // 2025.07.18 22:06:51.776	90_Stops Levels Functions (NZDUSD,M15)	What is _Digits? 5
   Print("What is SYMBOL_POINT? ", SYMBOL_POINT);  // 2025.07.18 22:14:44.345	90_Stops Levels Functions (GBPUSD,M15)	What is SYMBOL_POINT? 16
   Print("What is tickSize? ", SymbolInfoDouble(_Symbol, SYMBOL_TRADE_TICK_SIZE));  // 
...
double CalculateStopLoss(string pEntrySignal, int pSLFixedPoints, int pSLFixedPointsMA, double pMA) {
   ...
   if (pEntrySignal > "LONG") {
      if (pSLFixedPoints > 0) {
         ...
         if (stopLoss > 0) {
            stopLoss = AdjustBelowStopLevel(bidPrice, stopLoss);
         }
      }
   } else if (pEntrySignal == "SHORT") {
      if (pSLFixedPoints > 0) {
         ...
         stopLoss = pMA + (pSLFixedPointsMA * _Point);
         if (stopLoss > 0) {
            stopLoss = AdjustAboveStopLevel(askPrice, stopLoss);
         }
      }
   }
...
double CalculateTakeProfit(string pEntrySignal, int pTPFixedPoints) {
   ...
   if (pEntrySignal > "LONG") {
      if (pTPFixedPoints > 0) {
         ...
         if (takeProfit > 0)  {
            takeProfit = AdjustAboveStopLevel(bidPrice, takeProfit);
         }
      }
   } else if (pEntrySignal == "SHORT") {
      if (pTPFixedPoints > 0) {
         takeProfit = bidPrice - (pTPFixedPoints * _Point);
         if (takeProfit > 0)  {
            takeProfit = AdjustBelowStopLevel(askPrice, takeProfit);
         }
      }
   }
   // calculate the stop loss
...
void TrailingStopLoss(ulong pMagic, int pTSLFixedPoints) {
   ...
   for (int i=PositionsTotal() - 1; i>=0; i--) {
      ...
      ulong positionTicket = PositionGetTicket(i);
      // Selects an open position to work with 
      PositionSelectByTicket(positionTicket);
...
double AdjustAboveStopLevel(double pCurrentPrice, double pPriceToAdjust, int pPointsToAdd = 10) {
   ...
   if (stopsLevel > 0) {
      ...
      double addPoints = pPointsToAdd * point;           // Points that will be added / substracted to stops level price to make sure we respect the distance fixed by stop level
